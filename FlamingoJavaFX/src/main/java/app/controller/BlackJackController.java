...
public class BlackJackController implements Initializable {
	...
	public void btnHit_Click(ActionEvent event) {
		...
		patTMoveRot.getChildren().add(pathT); //.addAll(rotT, pathT);
	...
	private PathTransition CreatePathTransition(Point2D fromPoint, Point2D toPoint, ImageView img) {
		...
		path.getElements().add(new QuadCurveTo(toPoint.getX(), fromPoint.getY(), toPoint.getX()+33,
				toPoint.getY()+50));
	...
	private ParallelTransition createFadeTransition(final ImageView imgVFadeOut, final Image imgFadeIn) {
		...
		ScaleTransition sT = new ScaleTransition(Duration.millis(iAnimationLength),imgVFadeOut);
		sT.setToX(0f);
		sT.setCycleCount(2);
		sT.setAutoReverse(true);
		
		ParallelTransition parallelTransition = new ParallelTransition();
		parallelTransition.getChildren().addAll(fadeOutTransition,sT,fadeInTransition);
